...
-- are created separately via Supabase Auth + seed.sql.
-- ============================================================
...
-- Dental chart enums (findings-based model)
CREATE TYPE tooth_presence_type AS ENUM ('present', 'missing', 'impacted', 'unerupted');
CREATE TYPE tooth_finding_category AS ENUM ('condition', 'restoration', 'surgery');
CREATE TYPE tooth_surface AS ENUM ('mesial', 'distal', 'buccal', 'lingual', 'occlusal');
...
-- ============================================================
CREATE TABLE consent_form_clauses (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- ============================================================
CREATE TABLE payment_receipt_versions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_patients_messenger_psid ON patients (messenger_psid) WHERE messenger_psid IS NOT NULL;
...
CREATE INDEX idx_messenger_messages_unread ON messenger_messages (conversation_id, is_read) WHERE is_read = false;
...
-- Booking sessions
CREATE INDEX idx_booking_sessions_psid ON booking_sessions (patient_psid);
CREATE INDEX idx_booking_sessions_step ON booking_sessions (step);
CREATE INDEX idx_booking_sessions_created_at ON booking_sessions (created_at);

-- Patient medical records
CREATE INDEX idx_patient_medical_records_patient ON patient_medical_records (patient_id);
CREATE INDEX idx_patient_medical_conditions_patient ON patient_medical_conditions (patient_id);

-- Consent form clauses
CREATE INDEX idx_consent_form_clauses_form ON consent_form_clauses (consent_form_id);
