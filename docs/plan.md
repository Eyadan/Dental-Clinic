...
| P1-04 | Create database schema migrations (all 21 tables, 3NF) | ✅ Done | P1-03 | 21 initial tables, 8 enum types, all FKs, indexes, updated_at triggers, audit_log immutability. Later migrations added 10 more tables (31 total): `dental_charts`, `tooth_presence`, `tooth_findings`, `finding_surfaces`, `booking_sessions`, `medical_conditions`, `patient_medical_records`, `patient_medical_conditions`, `consent_clauses`, `consent_form_clauses` |
| P1-05 | Create RLS policies for all tables | ✅ Done | P1-04 | 31 tables RLS-enabled (21 initial + 10 from later migrations), role-based policies (admin/reception/dentist), helper functions, immutable tables protected |
...
| P3-13 | Treatment documentation (dental chart, clinical notes) | ✅ Done | P2-05 | Dental chart component with FDI/ISO 3950 numbering (32 teeth, 4 quadrants), 8 tooth statuses with color coding, interactive clickable teeth, legend; cn utility created. **Redesigned** to findings-based model: 3 normalized tables (tooth_presence, tooth_findings, finding_surfaces), multi-finding per tooth/surface, presence radio + surface multi-select + multi-select conditions/restorations/surgery, ToothEditor with existing findings list + delete + clear all. See DENTAL-CHART-REDESIGN in plan_done.md |
...
| P7-08c | Session 9 bug fixes (timezone, bookings dashboard, rescheduled appointments) | ✅ Done | P7-08b | Fixed 2 bugs: **BUG-063** (CRITICAL) — Timezone bug where `toISOString().split("T")[0]` converted to UTC, shifting dates back 1 day in UTC+8. Created `date-utils.ts` with `todayLocal()`, `formatDateLocal()`, `endOfMonthLocal()` and replaced 15+ instances across all date-filtered pages. **BUG-064** (HIGH) — Bookings "All Requests" tab showed 0 results because client component only filtered pending-only server data. Added `"all"` to server `validStatuses`, changed tab clicks to `router.push` for server refetch, added `key` prop for remount. |
| P7-09 | Production deployment (Vercel + Supabase Cloud + R2) | ⬜ Pending | P7-08c | |
